...
pub async fn search_in_files(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Sort results by relevance (match count) and then by path
        results.sort_by(|a, b| {
            ...
            // Then by path length (shorter paths often more relevant)
            let len_cmp = a.file.path.len().cmp(&b.file.path.len());
        ...
        // Limit results if too many files
        if results.len() > 100 {
...
pub async fn get_all_files(root_path: String) -> Result<Vec<SearchFile>, String> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Use WalkBuilder but disable all automatic ignore logic to show "ALL FILES" as requested
        ...
        for entry in walker {
            ...
            // Проверяем, не находимся ли мы ВНУТРИ игнорируемой папки
            let is_in_ignored = path.components().any(|c| {
                let s = c.as_os_str().to_string_lossy();
                s == "node_modules" || s == ".git" || s == "dist" || s == "build" || s == "target"
            });
            ...
            if entry.file_type().map(|ft| ft.is_file()).unwrap_or(false) {
                ...
                // Use the root_path_buf we already have
                let relative_path = path.strip_prefix(&root_path_buf)
                    .ok()
                    .and_then(|p| p.to_str())
                    .unwrap_or_else(|| path.to_str().unwrap_or(""))
                    .replace("\\", "/"); // Normalize path for frontend
...
pub async fn find_files_by_name(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let files: Vec<SearchFile> = WalkBuilder::new(&root_path)
            .hidden(false)
            .git_ignore(true)
            .git_global(true)
            .git_exclude(true)
            ...
            .filter(|e| {
                ...
                regex.is_match(file_name) || (!relative_path.is_empty() && regex.is_match(relative_path))
            })
            .map(|entry| {
                let path = entry.path();
                let relative_path = path.strip_prefix(&root_path_buf)
    ...
    .await
    .map_err(|e| format!("Find files task failed: {}", e))?
}
...
#[tauri::command]
pub fn rename_file_with_result(old_path: String, new_path: String) -> Result<RenameResult, String> {
    use crate::fs::types::RenameResult;
    
    // Check if it's a file before renaming
    let metadata = fs::metadata(&old_path).map_err(|e| e.to_string())?;
