...
export const BreadcrumbBar = ({ filePath, className }: BreadcrumbBarProps) => {
  ...
  const generatePathSegments = (filePath: string): BreadcrumbSegment[] => {
    ...
    // Нормализуем пути для корректного сравнения на Windows
    const normalizedFilePath = filePath.replace(/\\/g, '/');
    const normalizedWorkspace = currentWorkspace ? currentWorkspace.replace(/\\/g, '/') : '';

    if (currentWorkspace && normalizedFilePath.startsWith(normalizedWorkspace)) {
      relativePath = normalizedFilePath.slice(normalizedWorkspace.length);
      if (relativePath.startsWith('/') || relativePath.startsWith('\\')) {
        relativePath = relativePath.slice(1);
      }
    } else if (currentWorkspace) {
      // Если путь не начинается с workspace, пробуем найти общую часть
      const filePathParts = normalizedFilePath.split('/');
      const workspaceParts = normalizedWorkspace.split('/');
      
      // Находим индекс с которого пути начинают различаться
      let commonIndex = 0;
      for (let i = 0; i < Math.min(filePathParts.length, workspaceParts.length); i++) {
        if (filePathParts[i] === workspaceParts[i]) {
          commonIndex = i + 1;
        } else {
          break;
        }
      ...
      // Если нашли общую часть, используем её как базу
      if (commonIndex > 0) {
        relativePath = filePathParts.slice(commonIndex).join('/');
      }
    }
  ...
  const handleFolderClick = async (segment: BreadcrumbSegment, event: React.MouseEvent) => {
    if (segment.type === 'folder' && segment.fullPath) {
      ...
      // Position mini explorer below and slightly to the right of the clicked element
      const x = rect.left + 80; // Increased offset to the right //dont work
      const y = rect.bottom + 20; // Small gap below the clicked segment
