...
export const useEditorEvents = ({
    ...
    const collectDiagnostics = useCallback(() => {
        ...
        try {
            ...
            // Skip diagnostics for non-code files (like .gitignore, .env, .bat, etc.)
            ...
            const diagnostics: MonacoDiagnostic[] = markers.map((marker: any, idx: number) => {
                ...
                // Если есть тег Unnecessary (1), принудительно ставим тип hint
                if (marker.tags?.includes(1)) {
                    type = 'hint';
                } else if (marker.severity === 8) {
                    type = 'error';
    ...
    useEffect(() => {
        // Update last active file reference without clearing diagnostics
        // Diagnostics should persist for all open files
