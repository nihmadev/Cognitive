...
const GoToFileModal: React.FC<GoToFileModalProps> = ({ isOpen, onClose }) => {
    ...
    const recentFiles = React.useMemo(() => {
        return existingHistory.map(path => ({
            name: path.split(/[/\\]/).pop() || path,
            path: path,
            icon: getFileIcon(path.split(/[/\\]/).pop() || path, path)
        })).filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [existingHistory, searchQuery]);
    ...
    const displayFiles = React.useMemo(() => {
        return searchQuery.length > 0 ? filteredFiles : [...recentFiles, ...filteredFiles];
    }, [searchQuery, filteredFiles, recentFiles]);
    ...
    const checkHistoryExistence = async () => {
        const uniqueHistory = Array.from(new Set(history));
        const results = await Promise.all(uniqueHistory.map(async path => {
            try {
        ...
        setExistingHistory(results.filter((path): path is string => path !== null).reverse());
    };
                            ...
                            {searchQuery.length === 0 ? (
                                <>
                                    ...
                                    {filteredFiles.length > 0 && (
                                        <div>
                                            <div className={styles.recentTitle}>all files</div>
                                            {filteredFiles.map((file, idx) => {
                                                const actualIndex = recentFiles.length + idx;
                                                return (
                                                    <div
                                                        key={`all-${idx}`}
                                                        onClick={() => {
