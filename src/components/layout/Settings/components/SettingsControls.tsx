...
export const InputSetting = ({ label, description, value, type = 'text', placeholder, onChange }: { label: string; description?: string; value: string; type?: string; placeholder?: string; onChange: (v: string) => void }) => (
    <div className={styles.settingItemV2}>
        <div className={styles.settingLabelV2}>{label}</div>
        {description && <div className={styles.settingDescriptionV2} style={{ marginBottom: '8px' }}>{description}</div>}
        <input
            type={type}
