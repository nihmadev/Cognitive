...
export const useFolderGitStatus = (folderPath: string, isDirectory: boolean): FileGitStatus => {
    ...
    const status = useMemo((): FileGitStatus => {
        ...
        // Приоритет статусов: conflicted > deleted > modified > staged > untracked > ignored
        ...
        const hasDeleted = changedFiles.some(f => f.status === 'deleted');
        if (hasDeleted) {
            return { status: 'deleted', isStaged: false, hasConflicts: false, isIgnored };
        }
