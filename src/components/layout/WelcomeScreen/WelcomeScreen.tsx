...
export const WelcomeScreen = () => {
  const { setWorkspace, recentProjects } = useProjectStore();
                ...
                <div className={styles.recentList}>
                  {recentProjects.slice(0, 3).map((path) => (
                    <button 
                      ...
                      title={path}
                    >
                      <span className={styles.recentName}>{getProjectName(path)}</span>
                      <span className={styles.recentPath}>{path.split(/[\\/]/).slice(0, -1).join('/') || '/'}</span>
                    </button>
                  ))}
                </div>
              </div>
