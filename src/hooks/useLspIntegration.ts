...
export function useLspIntegration({
        ...
        const setupListener = async () => {
            const unlisten = await listenToLspDiagnostics((payload: LspDiagnosticsPayload) => {
                ...
                const diagnostics: MonacoDiagnostic[] = payload.diagnostics.map((d, index) => {
                    ...
                    const type = d.tags?.includes(1) ? 'hint' : (severityMap[d.severity] || 'error');
                    ...
                    if (model) {
                        ...
                        if (modelPath === filePath) {
                            const markers = diagnostics.map(d => ({
                                ...
                                message: d.message,
                                severity: d.tags?.includes(1) ? 1 : (d.type === 'error' ? 8 : d.type === 'warning' ? 4 : 2),
                                source: d.source,
                                code: d.code || undefined,
                                tags: d.tags?.map(tag => {
                                    if (tag === 1) { // Unnecessary
                                        const msg = d.message.toLowerCase();
                                        // Если это не импорт, добавляем тег Deprecated (2) для зачеркивания
    ...
    useEffect(() => {
        ...
        // Проверяем, поддерживается ли файл LSP (TypeScript/JavaScript/Python)
        const ext = activeFile.split('.').pop()?.toLowerCase();
        if (!ext || !['ts', 'tsx', 'js', 'jsx', 'py'].includes(ext)) {
            return;
        }
