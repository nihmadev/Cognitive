...
export const useEditorStore = create<EditorStore>((set, get) => ({
    ...
    runAction: (actionId: string) => {
        ...
        if (editorInstance) {
            try {
                // Ensure editor is focused before running action
                editorInstance.focus();
                
                // Use a small timeout to ensure focus is settled and menu is closed
                setTimeout(() => {
                    // Specific fix for Go to Definition: if it's not a TS/JS file,
                    // we might need to trigger it differently or ensure LSP is ready
                    const action = editorInstance.getAction(actionId);
                    if (action) {
                        action.run();
                    } else {
                        // Try to execute as a command if not an action
    ...
    jumpToLocation: (line: number, column: number) => {
        ...
        if (editorInstance) {
            ...
            editorInstance.revealPositionInCenter({ lineNumber: line, column: column });
            editorInstance.focus();
        }
    },
    getCurrentSymbol: () => {
