...
> = (set, get) => ({
    ...
    openSettingsTab: (section?: string | null) => {
        const { openSettingsTabs, activeFile, activeDiffTab, setActiveFile } = get();
    ...
    closeSettingsTab: (id: string) => {
        const { openSettingsTabs, activeSettingsTab, openFiles, openDiffTabs, setActiveFile } = get();
        ...
        } else if (!newActiveSettingsTab && !newActiveDiffTab) {
            setActiveFile(null);
        }
    },
